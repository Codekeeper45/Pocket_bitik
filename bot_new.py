...
async def ask_agentic(context: str, question: str, must_search: bool = False, caller: str = None, ctx_tokens_est: int = None, voice_mode: str = "off", images: list = None, chat_id=None, msg_by_id: dict = None) -> str:
    ...
    if has_web:
        system_prompt += ("\n\nУ тебя есть доступ в интернет: web_search (поиск), web_extract (прочитать страницы по URL), "
                          ...
                          "актуальные события, факты вне переписки, проверка спорных утверждений, ссылки из чата.\n"
                          "КАК искать качественно:\n"
                          "• Прежде чем встать на чью-то сторону или объявить утверждение из чата верным/ложным — "
                          "СОБЕРИ ПОЛНУЮ КАРТИНУ: не довольствуйся первым результатом. Сделай несколько запросов "
                          "под разными углами, при противоречии источников — проверь обе версии, открой первоисточник "
                          "через web_extract, сверь даты (свежее важнее).\n"
                          "• Не выноси вердикт «за/против» по одному сниппету. Мало/противоречиво данных — так и скажи, "
                          "не выдумывай уверенность.\n"
                          "• В ответе ссылайся на источники тегом <a href=\"URL\">. Не ищи то, что и так знаешь или что есть в чате.")
    if has_reply:
        system_prompt += ("\n\nКаждое сообщение в истории помечено его #id (число перед текстом). Инструмент "
                          ...
                          f"(до {REPLY_MAX}), каждый ответ отдельным сообщением, прикреплённым к своему.\n"
                          "КОГДА реплаить (а не один общий ответ):\n"
                          "• вопрос просит обратиться к людям/сообщениям адресно ("
                          "«ответь каждому», «разбери спор», «отреагируй на сообщения»);\n"
                          "• в чате несколько РАЗНЫХ адресатов/реплик, и общий ответ их смешал бы — дай каждому свой реплай;\n"
                          "• нужно привязать ответ к конкретному сообщению, чтобы было видно, на что отвечаешь.\n"
                          "КОГДА НЕ реплаить: вопрос общий/про чат в целом, или адресат один — хватит обычного ответа. "
                          "Не дроби искусственно и не спамь реплаями ради количества.\n"
                          "Можно подсветить КОНКРЕТНЫЙ фрагмент: в поле quote передай дословную подстроку этого "
                          "сообщения — в цитате выделится именно она (удобно для длинных). После реплаев ВСЕГДА дай "
                          "общий итоговый ответ обычным текстом, не дублируя сказанное в реплаях дословно.")
