...
def _voice_style_text(engine: str = "gemini", fish_model: str = "") -> str:
    """Инструкция по управлению голосом, режиссурой и стилем для ИИ."""
    ...
    # Gemini (дефолт) — полное управление голосом, Voice Design и эмоции
    vf = "Leda (юная, звонкая), Aoede (легкая), Kore (строгая), Zephyr (яркая), Callirrhoe (спокойная), Despina (мягкая), Sulafat (теплая), Autonoe (чистая)"
    vm = "Puck (бодрый, живой), Charon (глубокий диктор), Fenrir (эмоциональный, дерзкий), Orus (командный, твердый), Algenib (хриплый), Enceladus (придыхание), Iapetus (нейтральный), Umbriel (расслабленный)"
...
def _voice_auto_hint(engine: str = "gemini", fish_model: str = "") -> str:
    ...
    return (
        ...
        "По умолчанию отвечай ТЕКСТОМ по правилам выше (Telegram-HTML). НО если ответ уместнее, живее и эмоциональнее голосом "
        "(эмоция, шутка, личный разговор, отыгрыш роли, поддержка, или если пользователь просит голос) — "
        "ты можешь ответить ГОЛОСОВЫМ сообщением!\n"
        "Чтобы ответить голосом: начни самую первую строку с блока [[VOICE]] (выбрав любой подходящий голос и стиль под ситуацию, как описано ниже). "
        "Если голос не нужен — отвечай обычным текстом без блока [[VOICE]]."
...
# --- Озвучка ответов (Google Gemini Flash TTS · Voice Design) ---

VOICE_NAMES_CANONICAL = [
...
def _parse_voice_directive(raw_text: str, default_voice: str = "Leda", default_style: str = "") -> dict:
    ...
    if m:
        ...
        header_end = m.end()
        if inline_args and inline_args.lower() in VOICE_MAP_LOWER:
            voice = VOICE_MAP_LOWER[inline_args.lower()]
        elif inline_args:
        ...
        for line in lines:
            sline = line.strip()
            if in_header and (sline.startswith("---") or sline.startswith("===") or sline == "---"):
                in_header = False
                continue
            if in_header:
...
async def ask_agentic(context: str, question: str, must_search: bool = False, caller: str = None, ctx_tokens_est: int = None, voice_mode: str = "off", images: list = None, chat_id=None, msg_by_id: dict = None, memory_allowed: bool = True, asker_id=None) -> str:
    ...
    if has_web:
        system_prompt += ("\n\n━━ ДОСТУП В ИНТЕРНЕТ И ВЕБ-ИНСТРУМЕНТЫ ━━\n"
                          "У тебя есть полноценный доступ в интернет: web_search (поиск), web_extract (чтение страниц по URL целиком), "
                          "web_crawl (обход раздела сайта), web_map (карта ссылок сайта).\n"
                          "ПРОАКТИВНО используй веб-поиск — будь дотошным исследователем и фактчекером, не полагайся на слепую память:\n"
                          "1. Фактчекинг и проверка: если в чате звучат спорные факты, даты, цены, технические характеристики, релизы или новости — "
                          "не пытайся угадывать по памяти, обязательно вызывай web_search и проверяй факты. Оценивай объективно с опорой на пруфы.\n"
                          "2. Свежие новости и актуальность: мир меняется каждый день (новые версии ИИ, моделей, софта, события). Для любых вопросов "
                          "о текущем положении дел или свежих трендах ищи самые свежие данные и бенчмарки.\n"
                          "3. Поиск альтернатив и вариантов: если собеседники ищут совет, инструмент, библиотеку или решение проблемы — не ограничивайся "
                          "одним вариантом. Сходи в веб, найди 2–3 рабочие альтернативы, свежие обсуждения (Reddit, GitHub, профильные форумы) и сравни их.\n"
                          "4. Устранение пробелов и любопытство: если встречается незнакомый термин, проект, имя, мем или контекст, которого ты не знаешь "
                          "или в котором есть сомнения — сразу ищи в интернете всё, что нужно, чтобы дать глубокий и точный ответ.\n"
                          "5. Глубокое чтение: не ограничивайся поверхностными сниппетами поиска — читай полные страницы через web_extract для извлечения кода, деталей и инструкций.\n"
                          "6. Оформление: все источники аккуратно указывай кликабельными HTML-ссылками: <a href=\"URL\">название источника</a>.")
    if has_reply:
    ...
    elif voice_mode == "auto" or tts_available:
...
async def ask_command(event):
    ...
    # Проверяем явный запрос голоса в тексте вопроса (напр. «ответь голосом», «скажи голосовым», «озвучь»)
    explicit_voice_req = bool(re.search(r'(?i)\b(?:ответь|скажи|запиши|пришли|отправь)?\s*(?:голосом|голосовым|голосовух[а-я]|гс|voice)\b', question or ""))
    if explicit_voice_req:
        want_voice = True

    # Режим голоса для промпта: force (флаг -v или запрос в тексте) / auto (включён /voice auto) / off
    ...
    try:
        ...
        # Решаем, идёт ли ответ голосом: force (флаг -v или запрос в тексте), auto или модель выдала директиву [[VOICE
        is_voice_directive = bool(re.search(r'^\s*\[\[VOICE', reply, re.IGNORECASE))
        go_voice = False
        if voice_mode == "force" and tts_available:
            go_voice = True
        elif is_voice_directive and tts_available:
            go_voice = True
        elif voice_mode == "auto" and is_voice_directive and tts_available:
            go_voice = True
