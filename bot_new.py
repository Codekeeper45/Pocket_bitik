...
async def help_command(event):
    ...
    await event.edit(
        ...
        "╰───────────────────────╯\n"
        "\n"
        "💬 **ASK — вопрос к AI по чату**\n"
        "   `.ask N вопрос` — по последним N сообщениям\n"
        "   флаги: `-t` без медиа · `-c` обязательный поиск · `-d` дамп контекста в файл\n"
        "   `.ask N @user1 @user2 вопрос` — только от этих\n"
        "   `.ask N !@user вопрос` — исключить юзера (комбинируется с @user и флагами)\n"
        "   ↩️ в ответ на сообщение — вопрос про него + контекст\n"
        "\n"
        "🧠 **МОДЕЛЬ ОТВЕТОВ**\n"
        ...
        "\n"
        "📡 **КАНАЛЫ**\n"
        "   `.channels` — список · `scan` · `add N/@name` · `remove N/@name`\n"
        "   `.search запрос` — поиск по каналам (топ-10)\n"
        "   `.digest` — дайджест сейчас · `.digest time 09:00`\n"
        "\n"
        "🔁 **АВТО-ОТВЕТ** (с памятью диалога)\n"
        "   `.auto_reply` · `.auto_reply off`\n"
        "\n"
        "👥 **ДОСТУП К .ask**\n"
        "   `.allow @name [N|unlimited]` / в ответ — выдать · `.allow remove` — забрать\n"
        "\n"
