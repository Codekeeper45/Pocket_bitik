...
_IMAGE_GEN_WITH_REFS_SYSTEM = (
    ...
    "визуальный промпт на английском (композиция, стиль, свет, атмосфера). Ты МОЖЕШЬ выбрать подходящие "
    "изображения-референсы по их номерам и в самом промпте явно сказать, что с ними делать (взять персонажа/лицо, "
    "перенять стиль, использовать как фон, объединить несколько и т.п.). Бери только реально нужные (обычно 0–4 "
    "штуки). Если референсы не нужны — оставь список пустым.\n"
...
@client.on(events.NewMessage(pattern=r"^[./]gen(?:\s+(\d+))?((?:\s+-(?:improve|creative|vertical|horizontal|square|sq|4k|2k|1k|x\d+|noimg|ni|i|c|v|h))+)?((?:\s+!?@\w+)+)?\s+(.+)$"))
...
_HELP_SECTIONS = {
    ...
    "gen": (
        ...
        "\n"
        "**Синтаксис:** `/gen [N] [-i|-c] [-ni] [-v|-h|-sq] [-2k|-4k|-1k] [-xK] [@юзер|!@юзер] <промпт>`\n"
        ...
        "   (до 20 свежих), САМА выбирает подходящие как референсы и в промпте говорит, что с ними делать\n"
        "   (взять лицо/персонажа, стиль, фон, объединить). Vision-модель смотрит их напрямую, текстовая —\n"
        "   по описаниям. Выбранные уйдут генератору на вход. Отключить — флаг `-ni` (`/gen 200 -ni …`).\n"
        "\n"
