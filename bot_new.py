...
_VOICE_STYLE_COMMON = (
    ...
    "- Коротко-средне: до ~1500 символов (голосовое примерно на минуту-полторы). Без длинных списков — живо и по делу.\n"
...
def _voice_style_text(engine: str = "gemini", fish_model: str = "") -> str:
    ...
    if engine == "fish":
        # Fish S2 / s2-pro — [квадратные скобки] со СВОБОДНЫМИ описаниями подачи
        return _VOICE_STYLE_COMMON + (
            "- Управляй интонацией пометками в КВАДРАТНЫХ скобках — СВОБОДНЫМ описанием подачи (слово или фраза),\n"
            "  они НЕ произносятся: [шёпотом] [взволнованно и быстро] [смеётся] [с грустью] [саркастично]\n"
            "  [мягко, с теплотой] [серьёзно] [устало] [с придыханием]. Fish (s2) понимает ЛЮБЫЕ естественные\n"
            "  описания — не ограничивайся списком, будь выразительной и описывай эмоцию своими словами.\n"
            "- Пример: «[усмехается] Ну ты даёшь… [мягко, с теплотой] на самом деле — отличная идея.»"
        )
...
_HELP_SECTIONS = {
    ...
    "voice": (
        ...
        "   🎭 Разметку интонации модель расставляет САМА под движок: Gemini и Fish s2-pro — `[квадратные]`\n"
        "      описания (`[шёпотом]`, `[смеётся]`), Fish s1 — `(круглые)` из набора. Тебе делать ничего не нужно.\n"
        "\n"
