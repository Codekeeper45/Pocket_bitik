...
_HELP_SECTIONS = {
    ...
    "voice": (
        ...
        "   `.voice fish search <запрос>` — найти голоса Fish (покажет id+название).\n"
        "   `.voice fish add <id> [имя]` — добавить голос Fish в избранное; `.voice fish` — список/выбор.\n"
        "   `.voice fish <N|id>` — выбрать Fish-голос; `.voice fish test` — прослушать. Нужен `FISH_AUDIO_API_KEY`.\n"
        "\n"
    ...
    "keys": (
        ...
        "   `OYLAN_API_KEY` — провайдер ответов **Oylan (ISSAI)**, модель Oylan3.0 (`.model oylan`).\n"
        "\n"
