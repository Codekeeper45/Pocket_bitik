...
_IMAGE_GEN_WITH_REFS_SYSTEM = (
    "Ты — промпт-инженер для генерации изображений, работающий по логу чата. Тебе дан контекст чата и набор "
    "ДОСТУПНЫХ изображений из этого чата, пронумерованных #1, #2, … (показаны тебе напрямую и/или их текстовые "
    "описания). Эти изображения МОЖНО подать модели-генератору как референсы. Составь ОДИН финальный детальный "
    "визуальный промпт на английском (композиция, стиль, свет, атмосфера). Выбирай подходящие изображения-референсы "
    "по их номерам и в самом промпте явно говори, что с ними делать (взять персонажа/лицо, перенять стиль, "
    "использовать как фон, объединить несколько и т.п.).\n"
    "ВАЖНО про ПЕРСОНАЖЕЙ: если в запросе фигурируют люди/участники чата (например «нарисуй нас», «чатеры», "
    "конкретные ники/@упоминания, «пожелай им…»), а среди доступных изображений есть их фото — ОБЯЗАТЕЛЬНО возьми "
    "эти фото как референсы и в промпте укажи использовать ВНЕШНОСТЬ/ЛИЦО с конкретного номера (например "
    "'render character A using the face and appearance from image #3'). Это нужно для узнаваемости — не выдумывай "
    "внешность, если есть реальное фото человека. Бери все релевантные фото персонажей (можно несколько, до 16).\n"
    "Если же запрос абстрактный и подходящих фото нет — оставь список референсов пустым.\n"
...
_HELP_SECTIONS = {
    ...
    "gen": (
        ...
        "   • **`-ni`** — вообще не брать картинки из истории (`/gen 200 -ni …`, как раньше, только текст).\n"
        ...
        "\n"
        "**Режим `-c` (creative) — спросить модель, а не редактировать:**\n"
        "   Активная модель сама СОЧИНЯЕТ генеративный промпт-ОТВЕТ на твой запрос (даже когда есть референс),\n"
        ...
        "   (без `-c` и с референсом — режим РЕДАКТИРОВАНИЯ: только уточняет, без отсебятины).\n"
        "   Креативность: `-c` — свободнее и смелее (выше «температура»), `-i` — аккуратное уточнение твоего промпта.\n"
        "\n"
